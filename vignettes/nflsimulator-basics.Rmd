...
[`nflscrapR`](https://ryurko.github.io/nflscrapR-data/) and [`nflfastR`](https://github.com/guga31bb/nflfastR-data) by year. The `prep_pbp_data()` function will then clean and prepare the data for use in the sampling functions that are included in `NFLSimulatoR`.
...
Now, we can utilize the above sample play functions to simulate full drives using the `sample_drives_until_score()` function. The strategy options are the same as `sample_play()` with varying parameters based on the chosen strategy. The function also offers the option to simulate one single possession or until one team scores.

```{r}
drives <- NFLSimulatoR::sample_drives_until_score(
  n_sims = 10,
  from_yard_line = 25,
  play_by_play_data = pbp_data,
  strategy = "fourth_downs",
