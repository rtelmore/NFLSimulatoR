...
expected_pts_fourth <- function(yards_from_goal,
  ...
  df_miss <- dplyr::filter(play_data,
                    yardline_100 %in% `if`(yards_from_goal <= 20, 
                                           c(19:21), 
  ...
  df_punt_opp <- dplyr::filter(play_data, yardline_100 == avg_opp_ydln,down==1,ydstogo ==10)
