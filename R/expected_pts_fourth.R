...
expected_pts_fourth <- function(yards_from_goal,
  ...
  ev_goforit = p_gain * ep_gain + p_stopped * ep_stopped
  ...
  df_opp_fg <-
    filter(play_data, yardline_100 == 75, down == 1, ydstogo == 10)
  ...
  df_punt <-
    ...
    mutate(opp_yard_line = ifelse(
      touchback == 1,
      75,-1 * (yardline_100 - kick_distance - 100 + return_yards)
    ))
  avg_opp_ydln <- round(mean(df_punt$opp_yard_line, na.rm = TRUE))
  df_punt_opp <-
    filter(play_data, yardline_100 == avg_opp_ydln, down == 1, ydstogo == 10)
